...
begin : gen_soc_lc_assertions
  ...
  //
  // The ##2 is to allow a cycle for the connected LCC to respond to the change in security_state_o.
  `CALIPTRA_ASSERT(NoLcDftInProduction_A,
                   locked_in_soc_production |=> ##1 (lc_dft_en_i != lc_ctrl_pkg::On))
  ...
  `CALIPTRA_ASSERT(NoLcDbgInProduction_A,
                   locked_in_soc_production |=> ##1 (lc_hw_debug_en_i != lc_ctrl_pkg::On))
  ...
  `CALIPTRA_ASSERT(ProdSIGNAL_Decoding_A,
                   $rose(locked_in_soc_production) |=> ##1 !(SOC_DFT_EN || SOC_HW_DEBUG_EN))
